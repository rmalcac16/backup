...
class ApiController extends Controller
{
	...
	public function vipUsers(Request $request){
		try {
			return $this->user->vipUsers($request);
		} catch (Exception $e) {
			return array(
	            'msg' => $e->getMessage()
	        );
	...
	public function listAnimeFavorite(Request $request){
		try {
			$user = $this->user::find($request->user_id);
			$data = $user->getFavoriteItems(Anime::class)->select('id','name','poster')->orderBy('name','asc')->get();
			return $data;
		} catch (Exception $e) {
			return array(
	            'status' => false
